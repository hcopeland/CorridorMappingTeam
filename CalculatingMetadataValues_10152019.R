...
setwd("C:/Users/egelzer/Documents/MigrationMapper/MigrationMapperv1.3/Meeteetse_MD/Meeteetse_MD_Export") 
pointsout = "pointsOut_20190610151051.dbf"
mig.metadata = "metadata.csv"
winter.metadata = "metadata_winter.csv"
# ------------------------------------------------------------------------------------------------------------------------
# ------------------------------------------------------------------------------------------------------------------------
...
directory <-  paste0(getwd(),"/UDs")
fls_UDs_fall <- list.files(directory, pattern="fa")
fls_UDs_spring <- list.files(directory, pattern="sp")
...
mig.metadata <- read.csv(mig.metadata, stringsAsFactors=TRUE)
str(mig.metadata)
...
mig.metadata$Start.Date_1 <- as.Date (mig.metadata$Start.Date, format = "%m/%d/%Y")
mig.metadata$End.Date_1 <- as.Date (mig.metadata$End.Date, format = "%m/%d/%Y")
...
# Bring in metadata.csv
winter.metadata <- read.csv(winter.metadata, stringsAsFactors=TRUE)
str(winter.metadata)
